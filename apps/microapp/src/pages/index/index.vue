...
interface FoodCampaignListResponse {
  ok?: boolean;
...
const todayFoodCampaigns = ref<FoodCampaignSummary[]>([]);
...
const normalizeFoodCampaignStatus = (value: unknown) => {
...
const refreshTodayFoodCampaigns = async () => {
  if (backendProbeStatus.value !== "online" || !isAuthed.value) {
    todayFoodCampaigns.value = [];
    return;
  }
  try {
...
const refreshCurrentTabData = async (tab: TabKey) => {
  ...
  try {
    ...
    await requestAuthProfile().catch((error) => {
    ...
    if (tab === "today") {
      ...
      await refreshTodayFoodCampaigns();
      return;
    }
    if (tab === "schedule") {
...
onMounted(() => {
  ...
  void runBackendProbe().then(async (ok) => {
    ...
    void refreshTodayFoodCampaigns();
  });
...
watch(backendProbeStatus, (value) => {
  if (value === "online") {
    void refreshThemeImageMap();
    if (isAuthed.value) {
      void requestAuthProfile().catch((error) => {
        if (isAuthSessionInvalidError(error)) {
          clearAuthSession();
        }
      });
...
const profileDisplayProps = computed(() => ({
  ...
  profileAvatarUrl: resolveBackendMediaUrl(
    backendBaseUrl.value || DEFAULT_BACKEND_BASE_URL,
    String(socialDashboard.value?.me?.avatarUrl || authUser.value?.avatarUrl || "").trim(),
  ),
