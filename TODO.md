...
- `apps/backend/scripts/smoke-local.sh` 支持 `SMOKE_REAL_PDF_PATH` 真实 PDF 样本解析质量门禁，避免只验证伪 PDF 的错误终态；旧学号登录、提醒 token 检查、导入、社交 P0、提醒入队/ack 等本地专用链路会在任何请求前拒绝非 localhost / 127.0.0.1 的 `SMOKE_BASE_URL`，并提前校验学生号、提醒 header token、PDF 绝对路径、PDF 解析阈值、PDF 文件头、PDF 期望学号和导入轮询次数，且拒绝未替换占位符或 `/absolute/path/...` 模板路径；真实 PDF 路径不能缺导入学生号而被静默跳过，显式设置的 `SMOKE_REAL_PDF_EXPECT_STUDENT_NO` 必须匹配 `SMOKE_SCHEDULE_IMPORT_STUDENT_NO`，base URL 必须包含 scheme/host，且不能带空白字符、path、userinfo、query、fragment 或非法端口，避免 direct `smoke:local` 误打远端、请求目标不明确或带错 env 进入网络流程。
...
- `apps/backend/scripts/smoke-production.sh` 默认检查生产 bootstrap 管理员密码已初始化，并验证弱 fallback session token 被拒绝；可选 `TOUCHX_SMOKE_EXPECT_BOOTSTRAP_STUDENT_NO` 会在网络请求前拒绝未替换占位符和空白字符；即使设置 `TOUCHX_SMOKE_FALLBACK_ADMIN_PASSWORD`，也始终检查默认 `fallback:123456` 候选，避免漏掉历史弱默认，且该额外候选会拒绝未替换占位符和纯空白值；提供管理员 token 后可校验 `/api/v1/admin/me`，`TOUCHX_SMOKE_AUTH_LOGOUT=1` 会在其他 opt-in 检查后执行会让 token 失效的 logout 撤销 smoke；提供 `TOUCHX_SMOKE_STUDENT_NO` 后可非破坏性复核生产旧学号登录仍返回 `legacy_student_no`；提供 `TOUCHX_SMOKE_NOTIFICATION_QUEUE_MODE=1` 后可只读复核生产已有 `sourceQueue=notification` 的通用通知投递记录；提供 `TOUCHX_SMOKE_NOTIFICATION_CHANNELS` 后可循环验证 ClawDBot / 飞书外部投递，且会拒绝空通道段、重复通道和 legacy 单通道变量同时残留；legacy `TOUCHX_SMOKE_NOTIFICATION_CHANNEL` 只允许一个通道，多通道排障必须使用 `TOUCHX_SMOKE_NOTIFICATION_CHANNELS`；提供 `TOUCHX_SMOKE_CLAWDBOT_WEBHOOK=1` 和 webhook token 后可验证 ClawDBot webhook 入站链路；这些 opt-in flag 直接运行时只接受空值或 `1`，token 会拒绝未替换占位符，学生号会在网络请求前校验为 6-32 位数字，可选文案会拒绝未替换占位符和纯空白值，已开启 opt-in 的必需 token、学生号和通知通道也会在网络请求前校验，避免拼错值或 example env 被静默带进生产请求。
- `apps/backend/scripts/verify-v1-production.sh` 聚合 V1 生产验收：要求管理员 token、真实学生学号、真实 ClawDBot webhook token、ClawDBot + 飞书双通知通道、真实 PDF 样本与 Wrangler 登录态，且生产学生号、本地导入学生号和 PDF 期望学生号必须是 6-32 位数字并拒绝未替换占位符；先跑本地真实 PDF 解析 smoke，再串起 Cloudflare live 和生产 smoke；真实 PDF 默认要求至少 8 条课程且解析学号匹配真实学生号，预检会要求路径为绝对 PDF 文件路径，且要求生产学生号、本地导入学生号、PDF 期望学号三者一致，并要求 `SMOKE_REAL_PDF_MIN_ENTRIES` 不能是未替换占位符且大于等于 8；`SMOKE_BASE_URL` 必须保持 localhost / 127.0.0.1，避免本地导入 smoke 误写生产数据；`SMOKE_BASE_URL` 和 `TOUCHX_SMOKE_BASE_URL` 都必须包含 scheme/host 且不能带空白字符、path、userinfo、query、fragment 或非法端口；`TOUCHX_SMOKE_BASE_URL` 必须是公网 HTTPS，不能指向非 global IP literal、单标签 hostname、非法 DNS hostname、保留域名（example/test/invalid/local/localhost）、localhost / 127.0.0.1 / link-local / CGNAT / 私网 / 保留或文档地址，避免把本地、内网或保留地址 API 当作生产 API 验收；完整生产 gate 会拒绝 `TOUCHX_SMOKE_SKIP_SESSION_SECRET_CHECK`，也会拒绝已知 dummy/example/test token 或 secret 变体以及裸 `placeholder` / `changeme` 值，避免跳过弱 fallback session token 拒绝检查或误把默认 smoke 当完整上线验收；缺材料会直接失败。
- 最近通过的本地 release-candidate gate（2026-06-08）：`pnpm verify:v1-release`，覆盖 `pnpm --filter @touchx/backend verify:v1-local`、`@touchx/miniapp build:weapp`、`@touchx/microapp type-check` 和 `@touchx/microapp build:mp-weixin`；其中后端本地 gate 覆盖 backend type-check、后端 node tests、`pnpm test:packages` workspace 包测试、miniapp / mobile type-check、`smoke:api-boundaries`、`smoke:admin-ui-boundaries`、`smoke:client-boundaries`、`smoke:miniapp-parity`、`smoke:data-boundaries`、`smoke:cloudflare-config`、`apps/backend/scripts/*.sh` 全量 shell 语法检查、`git diff --check`。
- 最近通过的 V1 生产 smoke gate 增量验证（2026-06-08）：`node --test "apps/backend/server/services/smoke-production-script.test.mjs"` 共 62 个用例通过；`apps/backend/scripts/*.sh` 全量 `bash -n`、`git diff --check` 和 `pnpm --filter "@touchx/backend" verify:v1-local` 均通过。
