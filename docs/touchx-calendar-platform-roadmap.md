...
- [x] 今日 / 周视图覆盖真实 API 的 loading、empty、error、未登录、已登录状态，并由 `smoke:miniapp-parity` 固化代码级 gate。
